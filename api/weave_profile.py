...
# # # # Backend route for determining the states of one user in regard to another.
# # Right now this only deals with following. This will have to be expanded to fit blocking later.
# # Expects a POST request with a JSON. Details will be in 'api/README.md'.
# # Returns a JSON containing whether the current user is following (or blocking) the given user.
@weave_profile.route('/userstates/', methods=["POST"])
@jwt_required
def weave_get_userstates():
    ...
    if request.method == "POST":
        ...
        # Checks that JSON has all needed elements, then finds the selected user.
        if ("username" not in user_info):
            return jsonify({'error_message': 'Request Error: Missing JSON Element'}), 400
        given_user = user_info["username"]
        ...
        # Checks if the current user is following the given user.
        follow_query = "SELECT * FROM FollowUser WHERE user_follower = %s AND user_followed = %s;"
        follow_values = (current_user, given_user)
        cursor.execute(follow_query, follow_values)
        if (cursor.rowcount > 0):
            ret["follow"] = 1
