    ...
    if request.method == "POST":
        ...
        if (vote_info["type"] == 1):
            ...
            if (vote_info["vote"] == 1):
                ...
                cursor.execute("SELECT score FROM PostVote WHERE post_id = %s AND username = %s;", (save_info["id"], save_info["username"]))
                ...
                if (cursor.rowcount == 0):
                    ...
                    vote_values = (save_info["username"], save_info["id"], save_info["vote"])
                    cursor.execute(save_query, save_values)

                    # Updates the post table with the upvote.
                    cursor.execute("UPDATE Post SET upvote = upvote + 1 WHERE post_id = %s;", (save_info["id"],))
                ...
                # If so, return an error. If not, delete the downvote and change to an upvote.
                    ...
                    # If the old score was a downvote, remove it and add an upvote instead.
                        ...
                        cursor.execute("UPDATE PostVote SET score = 1 WHERE WHERE post_id = %s AND username = %s;", (save_info["id"], save_info["username"]))

                        # Removes downvote and adds upvote to the appropriate post.
                        cursor.execute("UPDATE Post SET downvote = downvote - 1, upvote = upvote + 1 WHERE post_id = %s;", (save_info["id"],))

                        # Seperate query for updating the post table with the upvote. Shouldn't be needed here.
                        # cursor.execute("UPDATE Post SET upvote = upvote + 1 WHERE post_id = %s;", (save_info["id"],))
            ...
            # Adds a downvote to the post.
                ...
                if (cursor.rowcount == 0):
                    ...
                    # Updates the post table with the upvote.
                    # THIS ONLY WORKS WITH NEW VOTES. MORE STUFF NEEDS TO BE WRITTEN HERE TO HANDLE CHANGED VOTES.
                    cursor.execute("UPDATE Post SET downvote = downvote + 1 WHERE post_id = %s;", (save_info["id"],))
            ...
            # Removes a vote from the post.
                ...
                cursor.execute("DELETE FROM PostVote WHERE post_id = %s AND username = %s;", (save_info["id"], save_info["username"]))
